...
DEFAULT_CONFIG = {
    ...
    # Whether the autoupdater is enabled or not.
...
def parse_args(conf=DEFAULT_CONFIG):
    ...
    parser.add_argument(
        "--disable-autoupdater",
        help="Disables the autoupdater for this build",
        action="store_true",
        default=conf["disable_autoupdater"])
...
def build(config):
    ...
    for arch in ARCHITECTURES:
        ...
        subprocess.check_call([
                ...
                "-DENABLE_AUTOUPDATE="
                + python_to_cmake_bool(not config["disable_autoupdater"]),
            ],
            env=env, cwd=arch)
